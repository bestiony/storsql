...
// build filter
if (isset($_GET['price'])) {
    $filter['price'] = $db->real_escape_string($_GET['price']);
}
if (isset($_GET['brand'])) {
...
// use the filter
if (isset($_GET['category']) && !empty($_GET['category']) || isset($_GET['brand']) && !empty($_GET['brand']) || isset($_GET['price']) && !empty($_GET['price'])) {
    $cash = array();

    $optionPrice = isset($_GET['price']) ? " price " . $db->real_escape_string($_GET['price'])  : " 2 > 1";
    $optionCategory = isset($_GET['category']) ? " category='" . $db->real_escape_string($_GET['category']) . "'" : " 2 > 1 ";
    $optionBrand = isset($_GET['brand']) ? " brand='" . $db->real_escape_string($_GET['brand']) . "'" : " 2 > 1";
    $seperator = " AND ";

    $filterQuery = "SELECT id FROM products WHERE" . $optionCategory . $seperator . $optionBrand . $seperator . $optionPrice;
            ...
            <div id="filterBar" class="tall-row" onblur="hideFilter()">
                ...
                <div class="row">
                    ...
                    foreach ($priceRanges as $range) {
                        $name = "BETWEEN $range[0] AND $range[1] ";
                        $cashFilter = $_SESSION['filter'];
                        $compare = isset($_GET['price']) ? $db->real_escape_string($_GET['price']) : "";
                        if ($name != $compare){
                            $cashFilter['price'] = $name;
                        } else {
                            unset ($cashFilter['price']) ;
                        }
                        // $cashFilter['price'] = $name != $compare ? $name : "";
                        $query = http_build_query($cashFilter);
                        $check = $name == $compare ? "fa-check-square-o" : "fa-square-o";
                        $link =  "Products.php?$query";
                        echo "<a href='$link'><i class='fa $check'></i> " . $range[0]."-".$range[1] . "</a>";
                    }
                    ?>
                </div>
                <h3>Categories</h3>
                <div class="row">
                    ...
                    foreach ($categories as $name) {
                        ...
                        if ($name != $compare){
                            $cashFilter['category'] = $name;
                        } else {
                            unset ($cashFilter['category']) ;
                        }
                ...
                <div class="row">
                    ...
                    foreach ($brands as $name) {
                        ...
                        if ($name != $compare){
                            $cashFilter['brand'] = $name;
                        } else {
                            unset ($cashFilter['brand']) ;
                        }
